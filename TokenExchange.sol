...
library SafeTRC20 {
    ...
    function callOptionalReturn(ITRC20 token, bytes memory data) private {
        ...
        require(address(token).isAContract(), "SafeTRC20: call to non-contract");
...
contract TokenExchange is Ownable {
    ...
    constructor(ITRC20 _sourceToken, ITRC20 _targetToken) public {
        sourceToken = _sourceToken; //BTT TNuoKL1ni8aoshfFL1ASca1Gou9RXwAzfn
        targetToken = _targetToken; //BTTNEW TKqF4eKxx6D7PU4iJ15bLMB4PcBk3nucUs
        blackHole = msg.sender;
    }

    function approve(uint256 amount) public {
        sourceToken.approve(address(this), amount);
    }
    ...
    function swap(uint256 amount) public {
        sourceToken.transferFrom(msg.sender, blackHole, amount);
    }
    ...
    function exchange(uint256 amount) payable public {
        ...
        sourceToken.safeTransfer(blackHole, amount);

        targetToken.safeTransfer(msg.sender, amount);

        emit Exchange(msg.sender, amount, amount);
    }
